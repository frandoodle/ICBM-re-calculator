...
Question for Holos team:
In Eq. 2.2.1-7, SoilOrganicC~Percent~ is defined as "Percentage of organic carbon in soil, by weight (%, SLC data table)".
I'm curious if this is planned to take its value from the ICBM model itself, calculated using bulk density?
</span>
...
Otherwise, a vector of daily irrigation values for the year can be passed into the `irrigation` parameter directly with `irrigation_use_estimate` set to `FALSE`:
...
`irrigation_use_estimate` needs to be specified, and can only take values of either `TRUE` or `FALSE`. If these conditions are not met, an error is returned:
...
## Testing tillage parameter (r~c~)

By default, r~c~ is given a value of 1.
This can either be changed based on Table 6 by specifying a `tillage_type` and a `tillage_soil`, or by supplying a vector of `r_c` values of length 365.
```{r}
calculate_re(daily_input_test,
...
try(calculate_re(daily_input_test,
						 ...
						 irrigation_use_estimate = FALSE))
```			 
If there is even one NA value in `r_c`, it is estimated using Table 6:
```{r}
try(calculate_re(daily_input_test,
...
# Running the code over multiple years

In order to run the script for multiple years, we need a daily input table with a `Year` column.
We can use `Year` as a grouping variable to run `calculate_re` for every year seperately.
...
(e.g. `suppressWarnings(calculate_re())`).
