...
# Quick Start
This code chunk represents the necessary input table and function call to use this r~e~ calculator.
...
GAI_return <- calculateGAI(daily_input_test$JulianDay,
													 daily_input_test$Yield,
													 daily_input_test$perennial)
GAI_return
GAI <- GAI_return[["GAI"]]
```
...
```{r}
WaterContent_return <- calculateWaterContent(daily_input_test$JulianDay,
																						 daily_input_test$SoilOrganicC_Percent[[1]],
																						 daily_input_test$ClayContent[[1]],
																						 daily_input_test$SandContent[[1]])
WaterContent_return
...
```{r}
SoilTemp <- calculateSoilTemp(daily_input_test_soiltemp$SurfaceTemp,
															daily_input_test_soiltemp$GAI,
															daily_input_test_soiltemp$SoilMeanDepth[[1]])
```
...
## Water Balance
Putting all required inputs in one table:
