...
## Test 1: Test data (1 year)

First we compare our intermediate calculations with Holos:
```{r}
...
## Test 2: NASA daily climate data (10 years)

```{r}
holos_output <- readr::read_csv(here("your_path.csv"), col_names=F) %>%
	pivot_longer(cols = 2:ncol(.)) %>%
	pivot_wider(names_from = `X1`) %>%
	select(-1)
holos_output_oneyear <- holos_output %>%
	filter(Year == 2000)
view(holos_output_oneyear)
```
