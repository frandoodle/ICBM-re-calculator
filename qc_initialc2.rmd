...
Assume the the measured SOC is evenly distributed from 0 to 30cm.
...
If we assume that the top 20cm of soil changes relative to treatment effect, but that the bottom 10cm remains relatively constant, then we could perform depth correction using a more "dynamic" approach.
...
site_data_control_comparison <- site_data_all %>%
	...
	group_by(location_name, is_control, soil_depth) %>%
...
ggplot(site_data_control_comparison, aes(y=soc, x=soil_depth, fill=is_control)) +
...
- For each treatment, sum every SOC depth measurement under 20cm deep. For instance, sum the measurements of 0-7.5cm and 7.5-15cm to calculate 0-15cm. Take this value and correct it to 0-20cm
...
- Sum the 0-20cm treatment SOC and the 20-30cm control SOC to get a corrected 0-30cm SOC estimate where the top 20cm are affected by changes in the treatment, and the lowest 10cm is set to the control SOC and therefore not swayed by treatment effects.
