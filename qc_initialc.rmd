...
1. Many of these experiments did not sample SOC at depths 0-30cm.
We corrected these values using the following algorithm:
1.1 For each treatment, sum every SOC depth measurement under 20cm deep. For instance, sum the measurements of 0-7.5cm and 7.5-15cm to calculate 0-15cm. Take this value and correct it to 0-20cm
1.2 Take the control value from the same experiment, find the single depth measurement closest to 20-30cm, and correct this measurement to 20-30cm
1.3 Sum the 0-20cm treatment SOC and the 20-30cm control SOC to get a corrected 0-30cm SOC estimate where the top 20cm are affected by changes in the treatment, and the lowest 10cm is set to the control SOC and therefore not swayed by treatment effects.
...
2. In order to visualize the change in SOC between the initial C estimate and the first measurement of SOC in the experiment, we need to decide on a year that the initial SOC estimate represents.
Here is my approach:
...
site_data_all_groups <- site_data_all %>%
	group_by(location_name, plot_id, block, treatment_number, treatment_name) %>%
	group_split
...
site_data_all_joined <- full_join(site_data_corrected, initialc_dsm, by="location_name") %>%
	...
	select(location_name,
				 initial_lte_master = initial_soc_tha, initial_dsm = `Can_SOC30 (Mg ha-1)`, initial_cmods = init_soc, initial_soc_year,
				 treatment_name,
				 year_name,
...
site_data_all_joined %>%
	...
	pivot_longer(c("initial_dsm", "initial_lte_master", "initial_cmods"), names_to="initial_method", values_to="initial") %>%
