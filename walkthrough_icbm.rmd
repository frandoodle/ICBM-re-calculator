...
ggplot(sa_ex_onesite, aes(x = reorder(params, -singsi), y = totsi, ymax=totsi.uci, ymin=totsi.lci))+ 
	xlab("Parameters") +
...
```{r}
uncertainty_ex_multipletrmt <- mc(site_data = sitedata_ex_multipletrmt,
													climate_data = climate_data,
...
We can graph this
```{r}
ggplot(uncertainty_ex_multipletrmt, aes(x=soc_total, y=soc_tha_30cm)) +
	geom_point()
```
