...
- For each treatment, sum every SOC depth measurement less than or equal to 30cm deep. For instance, sum the measurements of 0-7.5cm and 7.5-15cm to calculate 0-15cm. Take this value and correct it to 0-15cm
- Take the initial C estimate from the same experiment (0-30cm), and find the difference in SOC between the measured SOC and the initial SOC, which represents SOC for 15-30cm.
...
In order to avoid duplicating data which would affect the sample size of calibration datasets, we then average the SOC between depth groupings.
...
site_data_depth_groups_summed <- site_data_depth_groups %>%
	rename(lowest_depth = k) %>%
	group_by(Exp_ID, location_name, year_name, treatment_number, treatment_name, initial_soc_year, combination_group, lowest_depth) %>%
	...
	group_by(Exp_ID, location_name, year_name, treatment_number, treatment_name, initial_soc_year, lowest_depth) %>%
	summarise(across(c("soc_tha_lowest_depth", "initial_dataset"), mean))
```
